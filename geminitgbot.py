...
credentials_json_str = base64.b64decode(GOOGLE_CREDENTIALS_BASE64)
    .decode('utf-8') if GOOGLE_CREDENTIALS_BASE64
else None
CREDENTIALS = json.loads(credentials_json_str) if credentials_json_str
else None
if CREDENTIALS:
    logging.info("成功加载凭据。")
else :
    logging.warning("GOOGLE_CREDENTIALS 未加载。")# 确保将获取到的 GROUP_ID 转换为整数
try:
...
if row and row[0] == str(user_id):
    user_data = {
        ...
        , 'join_date': row[4]
        if len(row) > 4
        else datetime.datetime.now(datetime.timezone(datetime.timedelta(hours = 7)))
            .strftime('%Y-%m-%d')# 获取加入日期， 如果不存在则设置当前日期
    }
logging.info(f "get_user_info found existing user: {user_data}")
return user_data
except Exception as e:
    logging.error(f "get_user_info API error: {e}")
print(f "get_user_info API error: {e}")# 如果完全没有找到用户信息， 则写入新用户
if not user_data:
    new_user_data = [str(user_id), username, '3', '3', datetime.datetime.now(datetime.timezone(datetime.timedelta(hours = 7)))
        .strftime('%Y-%m-%d')]# 添加加入日期
body = {
...
response = service.spreadsheets()
    ...
    .append(
        spreadsheetId = SHEET_ID
        , range = SHEET_RANGE.split('!')[0], #只使用工作表名称 valueInputOption = 'RAW'
        , body = body
    )
    .execute()
logging.info(f "get_user_info added new user {user_id} to Google Sheets: {response}")
time.sleep(2)# 添加 2 秒延迟# 立即再次读取数据进行验证并打印
verification_result = service.spreadsheets()
    .values()
    .get(spreadsheetId = SHEET_ID, range = SHEET_RANGE)
    .execute()
...
return {
    ...
    , 'daily_limit': 3
    , 'remaining_days': 3
    , 'join_date': datetime.datetime.now(datetime.timezone(datetime.timedelta(hours = 7)))
        .strftime('%Y-%m-%d')
}
except Exception as e:
        ...
        , 'data': [
            {
                ...
                #行号应该是 i + 2， 假设次数在 C 列 'values': [[str(daily_limit)]]
                                }
                            ]
    }
update_result = service.spreadsheets()
    .values()
        ...
        , 'data': [
            {
                ...
                #假设剩余天数在 D 列 'values': [[str(remaining_days)]]
                                }
                            ]
    }
update_result = service.spreadsheets()
...
async def handle_feedback_message(update: Update, context: CallbackContext):
    ...
    user = update.effective_user
feedback_text = update.message.text# 这里可以将反馈发送给管理员或者保存到 Google Sheets
...
if user.id in ADMIN_IDS:
    service = get_sheets_service()
if service:
    range_name = f '{SHEET_RANGE.split("!")[0]}!A:C'#
获取用户 ID 和剩余次数
try:
result = service.spreadsheets()
    .values()
...
if values:
    ...
    body = {
        ...
        , 'data': [
            {
                'range': f 'UserStats!C{i + 2}',
                #假设次数在 C 列， 从第二行开始 'values': [[str(new_limit)]]
                                }
                            ]
    }
update_result = service.spreadsheets()
...
await save_translation_history(user_id, user_text, clean_text(full_translation.group(1)
        .strip()
        .replace('。', '\n')) if full_translation
    else '翻译失败')
else :
...
keyboard = [
            ['账号出售', '网站搭建', 'AI创业'], ['网赚资源', '常用工具', '技术指导'], ['翻译开关', '我的资料']
        ]
...
print(f "context.application.user_data for chat {update.effective_chat.id}: {context.application.user_data.get(update.effective_chat.id)}")
if button_text == '查看统计':
    await admin_stats(update, context)
elif button_text == '设置次数':
    context.user_data.setdefault(update.effective_chat.id
...
async def handle_admin_input(update: Update, context: CallbackContext):
    user = update.effective_user
print(f "handle_admin_input called for user {user.id} with text: {update.message.text}")
print(f "context.user_data in handle_admin_input: {context.user_data.get(update.effective_chat.id)}")
if context.user_data.get(update.effective_chat.id
        , {})
    .get('expecting_admin_set_limit'):
...
context.user_data.get(update.effective_chat.id
        , {})
    .pop('expecting_admin_set_limit', None)
elif context.user_data.get(update.effective_chat.id
        , {})
    .get('expecting_admin_broadcast'):
    message = update.message.text
await admin_broadcast(update, context, [message])
context.user_data.get(update.effective_chat.id
        , {})
    .pop('expecting_admin_broadcast', None)
...
prompt = f "从以下分类中随机生成 1 个老挝语词汇或句子，并提供中文翻译和拉丁语发音。分类：{', '.join(selected_categories)}。格式：中文：老挝语（谐音用汉语拼音）。已发送的词汇/句子：{sent_vocabulary}"
genai.configure(api_key = get_current_api_config()['api_key'])
model = genai.GenerativeModel(get_current_model())
response = model.generate_content(prompt)
...
return user_id in ADMIN_IDS and(
    message.chat.id in context.user_data and(
        context.user_data[message.chat.id].get('expecting_admin_set_limit') is True or context.user_data[message.chat.id].get('expecting_admin_broadcast') is True
    )
)
...
print(f "Admin {update.effective_user.id}: expecting_admin_broadcast set to True in admin_button_click")
        ...
        , {})
    .get('expecting_admin_set_limit') and update.message.text != '设置次数':
    await handle_admin_input(update, context)
elif context.user_data.get(update.effective_chat.id
        , {})
    .get('expecting_admin_broadcast') and update.message.text != '发送广播':
    await handle_admin_input(update, context)
return None
